...
# **************************************************************************************
# *********** Implementation of modified BFS that is about to be deleted soon **********
# **************************************************************************************
...
# def apply_BFS_modified(self, source_node, targets, network, dict):
#
...
#         neighbors_list = network.edges([str(this_node)])
#         print("neighbors: ", neighbors_list)
#
#         for edge_i in neighbors_list:
#             node_from, node_to = edge_i
#             current_weight = float(network.get_edge_data(*edge_i)['weight'])
#
#             if int(node_to) in targets:
#                 # print(int(node_to)," is in targets ", targets)
...
#                                                             answer_info.depth + 1)
#
#     for i,j in self.answer_map.items():
#         print("answer map: ",i, j.weight_so_far)
#     selected_node, selected_max_weight = self.find_correct_answer()
#     path = self.get_path(selected_node, source_node)
#     path_list = [int(s) for s in path.split(',')]
#     # print("HEYYYYYY: ", (path_list))
#
#     # Calculate the path cost
#     for i in range(len(path_list) - 1):
#         key = "(" + str(path_list[i]) + "," + str(path_list[i + 1]) + ")"
#         if key in dict:
#             final_cost += round(1 / float(dict[key]), 0)
#         else:
#             key = "(" + str(path_list[i + 1]) + "," + str(path_list[i]) + ")"
#             final_cost += round(1 / float(dict[key]), 0)
#     print("Selected path: ", path, " with final_cost: ", final_cost)

# def find_correct_answer(self):
#     selected_node = 0
#     selected_max_weight = 0
#     selected_depth = 10000
#     print("length of self.unique_list_answers_found: ", len(self.unique_list_answers_found))
#
#     for i in self.unique_list_answers_found:
#         # print("i: ", i)
#         ai = self.answer_map[i]
#         # print("ai weight: ", ai.weight_so_far, " with depth: ", ai.depth)
#         if ai.depth < selected_depth:
#             # print("11 selected node now is : ", i)
#             selected_node = i
#             selected_depth = ai.depth
#             selected_max_weight = ai.weight_so_far
#         elif ai.depth == selected_depth:
#             if ai.weight_so_far > selected_max_weight:
#                 # print("22 selected node now is : ", i)
#                 selected_node = i
#                 selected_depth = ai.depth
#                 selected_max_weight = ai.weight_so_far
#         else:
#             continue
#
#     print("Chosen node is: ", selected_node, " with cost: ", selected_max_weight)
